...
Деплой: прод — контейнер в Dokploy на сервере 192.168.0.42 (панель https://dokploy.infomania.ru), собирается из `master` по `Dockerfile`. Деплой — **автодеплой Dokploy**: GitHub App сообщает о push в `master` (вебхук `dokploy.infomania.ru/api/deploy/github`), Dokploy сам собирает образ и перезапускает контейнер. GitHub Actions в проекте нет (удалены 2026-09-26 вместе с раннерами на 192.168.0.35). Ход сборки и логи — в Dokploy → приложение → Deployments; ручной запуск — кнопка Deploy. Переменные окружения контейнера меняются в Dokploy (Environment). Порт приложения в Dokploy — в режиме `ingress`: в режиме `host` обновления зависают в Pending.
