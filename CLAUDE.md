...
**Dev-сервер:** перед запуском `npm run dev` всегда проверять, не запущен ли он уже на порту 3000 (например, `curl -s -o /dev/null -w '%{http_code}' localhost:3000/robots.txt`), и если запущен — использовать его, а не запускать второй. **Не останавливать** dev-сервер в конце работы — оставлять запущенным.
...
Деплой: прод — контейнер в Dokploy (http://192.168.0.35:3000, проект «infomania.ru» → `frontend`), собирается из `master` по `Dockerfile`, порт хоста 3006. Dokploy доступен только из локальной сети, поэтому уведомления GitHub о push до него не доходят: деплой запускает GitHub Actions (`.github/workflows/deploy.yml`, задание `deploy`) — раннер на том же сервере дёргает deploy-webhook приложения (адрес с токеном в `~/.config/dokploy/frontend-deploy-hook` на сервере). Линт идёт отдельным заданием и деплой не блокирует. Порт приложения в Dokploy — в режиме `ingress`: в режиме `host` новый контейнер не может стартовать, пока старый держит порт, и обновления зависают в Pending. Переменные окружения контейнера меняются в Dokploy (Environment), не в CI. pm2-процесс `frontend-v2` на сервере оставлен остановленным для отката — не запускать его, пока работает контейнер (конфликт за порт 3006).
